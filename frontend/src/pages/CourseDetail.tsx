...
const COURSE_META: Record<string, { title: string; description: string }> = {
  N5: {
    ...
    description: "Khởi động với các chủ đề nền tảng như chào hỏi, mẫu câu cơ bản và từ vựng thiết yếu.",
  },
  N4: {
  ...
  N1: {
    ...
    description: "Lộ trình chuyên sâu cho sắc thái nghĩa, cấu trúc khó và khả năng đọc hiểu tinh tế.",
  },
};
...
const CourseDetail = () => {
  ...
  const lessons = (allLessons as LessonSummary[]).filter((lesson) => (lesson.jlptLevel || "N5") === level);
  const progressByLessonId = new Map(progressItems.filter((item) => item.lessonId != null).map((item) => [item.lessonId as number, item]));
    ...
    <DashboardLayout>
      <div className="mx-auto max-w-[1380px]">
        ...
        <div className="mb-4 grid gap-3 md:grid-cols-3">
          <MetricCard hint="Tổng số bài đã xuất bản" icon={<BookOpen className="h-4 w-4 text-sky-500" />} label="Bài học" value={lessons.length} />
          <MetricCard hint="Ước tính 20 phút mỗi bài" icon={<Clock className="h-4 w-4 text-violet-500" />} label="Thời lượng" value={`${totalHours} giờ`} />
        ...
        <div className="grid gap-4 xl:grid-cols-[320px_minmax(0,1fr)]">
          <PageSection title="Tổng quan khóa học" description="Tóm tắt nhanh để bạn nắm bố cục toàn khóa trước khi vào bài.">
            <Badge className="rounded-full border border-sky-200 bg-sky-50 text-sky-700">JLPT {level}</Badge>
          ...
          <PageSection title="Danh sách bài" description="Danh sách này lấy từ backend, nên nút học sẽ mở đúng lesson thật.">
            {isLoading ? (
                    ...
                    <div key={lesson.id} className="rounded-[22px] border border-white bg-card p-4 shadow-[0_10px_24px_rgba(148,163,184,0.10)]">
                      <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                        <div className="flex min-w-0 items-center gap-4">
                          <div className={`flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl ${completed ? "bg-emerald-50 text-emerald-600" : "bg-sky-50 text-sky-600"}`}>
                            {completed ? <CheckCircle2 className="h-5 w-5" /> : <span className="text-sm font-semibold">{index + 1}</span>}
                          </div>
                          ...
                          <div className="min-w-0">
                            ...
                            <div className="mt-1 flex flex-wrap items-center gap-3 text-sm text-muted-foreground">
                              ...
                              {progressItem?.status && <span>{progressItem.status.replace("_", " ").toLowerCase()}</span>}
                            </div>
                            {lesson.description && <p className="mt-2 line-clamp-2 text-sm text-muted-foreground">{lesson.description}</p>}
                          </div>
                        </div>

                        <Link to={`/lessons/${lesson.id}`}>
                          <Button className={completed ? "rounded-2xl border-border bg-white text-foreground/80" : "rounded-2xl bg-primary text-primary-foreground hover:bg-primary/90"} variant={completed ? "outline" : "default"}>
