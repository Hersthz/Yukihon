...
import { friendApi, privateChatApi, type UserConnection, type ConnectionStatus, type PrivateMessage } from "@/api";
        ...
        <Dialog open={open} onOpenChange={setOpen}>
            ...
            <DialogContent className="sm:max-w-[425px] h-[80vh] flex flex-col p-0 gap-0">
                <DialogHeader className="p-4 border-b">
                    ...
                    <DialogDescription className="sr-only">Messaging and friends management</DialogDescription>
                </DialogHeader>
                <Tabs defaultValue="friends" className="flex-1 flex flex-col">
                    <div className="px-4 pt-2">
                        <TabsList className="grid w-full grid-cols-2">
                            ...
                            <TabsTrigger value="chat" disabled={!selectedFriendId}><MessageSquare className="w-4 h-4 mr-2"/> Chat</TabsTrigger>
                        </TabsList>
                    </div>
                                ...
                                <div>
                                    ...
                                    <div className="flex gap-2">
                                        <Input 
                                            ...
                                            type="number"
                                        />
                                        <Button size="icon" onClick={handleSendRequest}><UserPlus className="w-4 h-4"/></Button>
                                    </div>
                                </div>

                                {pending.length > 0 && (
                                    <div>
                                        <h4 className="font-semibold text-sm mb-2">Pending Requests ({pending.length})</h4>
                                        <div className="space-y-2">
                                            {pending.map(p => (
                                                <div key={p.id} className="flex items-center justify-between p-2 rounded-lg border bg-card">
                    ...
                    <TabsContent value="chat" className="flex-1 flex flex-col m-0 p-0 overflow-hidden outline-none">
                        {selectedFriendId ? (
                            <PrivateChatView 
...
const PrivateChatView = ({ currentUserId, otherUserId, otherUserName }: { currentUserId?: number; otherUserId: number; otherUserName: string }) => {
    ...
    return (
        <div className="flex flex-col h-full bg-background">
            <div className="p-3 border-b bg-muted/20 flex items-center justify-between">
                ...
                <span className={`w-2 h-2 rounded-full ${connectionState === 'connected' ? 'bg-green-500' : 'bg-red-500'}`} />
            </div>
            ...
            <div className="p-3 border-t bg-card">
                <form onSubmit={handleSend} className="flex gap-2">
                    <Input 
                        ...
                        className="flex-1"
                    />
                    <Button type="submit" disabled={!inputText.trim() || connectionState !== 'connected'}>Send</Button>
                </form>
            </div>
        </div>
    );
