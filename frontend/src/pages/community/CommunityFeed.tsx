      ...
      <div className="space-y-3">
          ...
          return (
            <div key={post.id} className="rounded-[22px] border border-white bg-card p-4 shadow-[0_10px_24px_rgba(148,163,184,0.10)]">
              <div className="mb-3 flex items-start justify-between gap-3">
                ...
                <div className="flex flex-wrap items-center justify-end gap-2">
                  ...
                  {currentUserId === post.userId ? (
                    <Button className="h-8 w-8 rounded-xl" onClick={() => onDeletePost(post.id)} size="icon" variant="ghost">
              ...
              <div className="mt-4 flex items-center gap-2 border-t border-border/80 pt-3">
                ...
                <Button className="rounded-xl text-muted-foreground hover:text-foreground" onClick={() => onToggleComments(post.id)} size="sm" variant="ghost">
                  ...
                  <motion.div animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} initial={{ opacity: 0, height: 0 }} className="overflow-hidden">
                    <div className="mt-4 space-y-3 border-t border-border/80 pt-3">
                      <div className="flex gap-2">
                        ...
                        />
                        <Button className="rounded-xl bg-primary text-primary-foreground hover:bg-primary/90" disabled={!(commentInputs[post.id] || "").trim()} onClick={() => onSubmitComment(post.id)} size="icon">
        ...
        {totalPages > 1 ? (
          <div className="flex items-center justify-center gap-2 pt-2">
            <Button className="rounded-xl" disabled={page === 0} onClick={() => onPageChange(page - 1)} size="sm" variant="outline">
              Truoc
            </Button>
            <span className="text-xs text-muted-foreground">
              {page + 1} / {totalPages}
            </span>
            <Button className="rounded-xl" disabled={page >= totalPages - 1} onClick={() => onPageChange(page + 1)} size="sm" variant="outline">
              Sau
            </Button>
          </div>
        ) : null}
