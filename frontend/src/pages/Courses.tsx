...
const courseMeta: Record<string, CourseMeta> = {
  N5: {
    ...
    description: "Bắt đầu từ nền tảng: chào hỏi, mẫu câu cơ bản, từ vựng thiết yếu và nhịp học đầu tiên.",
  ...
  N2: {
    ...
    description: "Đi sâu vào ngữ cảnh công việc, tin tức, văn bản dài và cách diễn đạt chính xác hơn.",
    skills: ["Reading", "Kanji", "Listening"],
  },
  N1: {
    id: "N1",
    title: "N1 Complete Mastery",
    description: "Lộ trình nâng cao cho sắc thái nghĩa, đọc hiểu chuyên sâu và năng lực ngôn ngữ tinh tế.",
    skills: ["All skills"],
  },
};
...
const Courses = () => {
  ...
  const courses = useMemo(() => {
    const lessonsByLevel = (allLessons as LessonSummary[]).reduce<Record<string, LessonSummary[]>>((acc, lesson) => {
    ...
    return Object.values(courseMeta).map((meta) => {
      ...
      const completed = lessons.filter((lesson) => progressByLessonId.get(lesson.id)?.status === "COMPLETED").length;
  ...
  const filteredCourses = useMemo(() => {
    return selectedLevel === "all" ? courses : courses.filter((course) => course.id === selectedLevel);
    ...
    <DashboardLayout>
      <div className="mx-auto max-w-[1420px]">
        ...
        <div className="mb-4 grid gap-3 md:grid-cols-3">
          <MetricCard hint="Theo các cấp JLPT có trong hệ thống" icon={<BookOpen className="h-4 w-4 text-sky-500" />} label="Khóa học" value={courses.length} />
          ...
          <MetricCard hint="Có tiến độ học từ backend" icon={<Sparkles className="h-4 w-4 text-emerald-500" />} label="Đang theo học" value={activeCourses} />
        </div>

        <PageSection className="mb-4" title="Bộ lọc level" description="Chọn cấp JLPT để lọc danh sách khóa học.">
        ...
        <PageSection title="Danh sách khóa học" description="Mỗi khóa gom các lesson cùng cấp JLPT, có tiến độ và đường vào bài học thật.">
          {isLessonLoading ? (
                ...
                <div key={course.id} className="yukihon-card-flat p-4 cursor-default">
                  <div className="mb-3 flex items-start justify-between gap-3">
                    <div>
                      ...
                      <Badge className={`mt-2 rounded-full border ${levelTone[course.id] || "border-border bg-muted text-foreground"}`}>
                        {course.level}
                      </Badge>
                    </div>
                    {course.progress > 0 && (
