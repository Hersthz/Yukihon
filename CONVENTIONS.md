...
- **Hook CRUD tự sinh:** REST thuần → `createResource<T>("/api/x")` + `createResourceHooks("x", api)` (`@/lib/createResourceHooks`). `@AutoCrud` (`/api/auto/*`, phân trang) → `createAutoCrudApi<T>("path")` + `createAutoCrudHooks("path", api)` (`@/lib/createAutoCrudHooks`, list trả `Page<T>`). Cả hai cho `useList/useGet/useCreate/useUpdate/useRemove`, mutation tự `invalidateQueries`. Thêm 1 resource = 2 dòng.
- **Admin CRUD generic:** `@AutoCrud` endpoint là 1 route generic `/api/auto/{resource}` (row = `object`, không type tĩnh từ spec). `MetadataDrivenCrudPage` render form/bảng từ `/api/meta` → màn quản trị CRUD **zero-code** cho mọi entity; truyền `Schema<"XxxDto">` vào factory chỉ khi tự dựng màn typed riêng.
