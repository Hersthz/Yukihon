...
public class LearningAnalyticsService {
    ...
    public StudyCalendarDto getStudyCalendar(Long userId, String startDate, String endDate) {
        ...
        UserLearningStats stats = userLearningStatsRepository.findByUserId(userId).orElse(null);
        ...
        List<StudyCalendarDayDto> days = rangeFilter.getStartDate()
                ...
                .map(date -> {
                    ...
                    long totalMinutes = projection == null ? 0L : toRoundedMinutes(safeLong(projection.getTotalDurationSeconds()));
                    ...
                    return StudyCalendarDayDto.builder()
                            ...
                            .isDeadlineDay(deadlinePlan != null && deadlinePlan.getDeadlineDate() != null && date.equals(deadlinePlan.getDeadlineDate()))
        ...
        int activeDays = (int) days.stream().filter(StudyCalendarDayDto::isHasActivity).count();
        long totalStudyEvents = days.stream().mapToLong(StudyCalendarDayDto::getTotalEvents).sum();
        long totalStudyMinutes = days.stream().mapToLong(StudyCalendarDayDto::getTotalMinutes).sum();
        StudyCalendarDayDto bestDay = days.stream()
                .filter(StudyCalendarDayDto::isHasActivity)
                .max(Comparator
                        .comparingLong(StudyCalendarDayDto::getTotalMinutes)
        ...
        return StudyCalendarDto.builder()
                ...
                .recommendedMinutesPerDay(deadlinePlan != null && deadlinePlan.getRequiredMinutesPerDay() > 0
    ...
    private List<LocalDate> buildRecommendedStudyDates(
        ...
        if (deadlinePlan != null && deadlinePlan.getDeadlineDate() != null && deadlinePlan.getDeadlineDate().isBefore(effectiveEnd)) {
        ...
        while (!cursor.isAfter(effectiveEnd)) {
            ...
            weekCandidates.stream()
                    .sorted(Comparator
                            .comparing((LocalDate date) -> date.getDayOfWeek() == DayOfWeek.SATURDAY || date.getDayOfWeek() == DayOfWeek.SUNDAY)
